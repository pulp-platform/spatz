...
set checkpoint_list [extract_stages "dp.tcl"]
...
# you want to skip a couple of specific checkpoints. The $start_from_checkpoint
# will skip everything up-to the checkpoint which might not be the behavior you
# want.
set SKIP_STAGES {
    block_shaping
    clock_trunk
...
# This can be achieved by setting exit_after_checkpoint from the Makefile e.g.
global TO_STAGE
if {![info exists TO_STAGE]} {set TO_STAGE {}}

# A single checkpoint name:
# If load_label is not overriden, start_from_checkpoint will load the block
# label of the corresponding checkpoint (e.g. after_elaborate.design). However
# you can specify your own label if needed.
#
# global load_label
# set load_label elaboration
...
# start_from_checkpoint feature will not send a notification regardless the
...
create_checkpoint_action save_block {
    global DESIGN
    save_block -hier -as $DESIGN/[get_current_checkpoint -name]
    save_lib -all
}
