...
// FUNC State Machine
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        ...
        sin_cos_state_q <= sin_cos_state_d;
      end
  end
  // ----------------------------------
  // Spill Registers 
  // ----------------------------------
  // Breaks the combinational loop: ADDMUL/CONV_valid -> CTRL_ready -> ADDMUL/CONV_ready
  ...
  assign addmul_out_valid_str = addmul_out_valid_i && (op_i == TANHS || op_i == RSQRT || op_i == SIN ||op_i == COS);
  ...
  assign conv_out_valid_str = conv_out_valid_i && (op_i == SIN ||op_i == COS );
  ...
  always_comb begin : in_ready_logic
    unique case (op_i)
        ...
        SIN, COS: block_issue_nl = (~(sin_cos_state_q == SIN_COS_RR_U     || sin_cos_state_q == SIN_COS_POLY5_L));
  ...
  always_comb begin : state_control
      ...
      unique case (sin_cos_state_q)
        ...
        SIN_COS_RR_L:         if (in_valid_i)                       sin_cos_state_d = SIN_COS_INT_CONV_U;
        ...
        SIN_COS_FLOAT_CONV_U: if (conv_out_valid_i)                 sin_cos_state_d = SIN_COS_FLOAT_CONV_L;
        SIN_COS_FLOAT_CONV_L: if (conv_out_valid_i)                 sin_cos_state_d = SIN_COS_POLY1_U;
        ...
        SIN_COS_POLY2_U:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY2_L;
        SIN_COS_POLY2_L:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY3_U;
        SIN_COS_POLY3_U:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY3_L;
        SIN_COS_POLY3_L:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY4_U;
        SIN_COS_POLY4_U:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY4_L;
        SIN_COS_POLY4_L:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY5_U;
        SIN_COS_POLY5_U:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_POLY5_L;
        SIN_COS_POLY5_L:      if (addmul_out_valid_i)               sin_cos_state_d = SIN_COS_DRAIN_U;
  ...
  always_comb begin : buffer_control
    ...
    SIN,COS: begin 
        nl_wr_en_0          = (sin_cos_state_q == SIN_COS_RR_U) && in_valid_i || ((sin_cos_state_q == SIN_COS_POLY2_U) && addmul_out_valid_i)|| ((sin_cos_state_q == SIN_COS_POLY5_U) && addmul_out_valid_i);
        nl_wr_en_1          = (sin_cos_state_q == SIN_COS_RR_L) && in_valid_i || ((sin_cos_state_q == SIN_COS_POLY2_L) && addmul_out_valid_i)|| ((sin_cos_state_q == SIN_COS_POLY5_L) && addmul_out_valid_i);
        nl_wr_en_2          = ((sin_cos_state_q == SIN_COS_POLY3_U) && in_valid_i);
        nl_wr_en_3          = ((sin_cos_state_q == SIN_COS_POLY3_L) && in_valid_i);
        ...
        SIN,COS:begin
            unique case(sin_cos_state_q)
                ...
                SIN_COS_POLY1_U, SIN_COS_POLY1_L: begin 
                    ...
                    addmul_operands_o[2] = SIN_COS_POLY1_U ? nl_intermediate_0_q : nl_intermediate_1_q;
                    addmul_rnd_mode_o    = fpnew_pkg::RNE;
                    addmul_op_o          = fpnew_pkg::FNMSUB;
                    addmul_op_mod_o      = 1'b0;
                    addmul_tag_o         = conv_tag_result_i;
                ...
                SIN_COS_POLY4_U, SIN_COS_POLY4_L: begin 
                    addmul_operands_o[0] =  addmul_res_q.result;       
                    addmul_operands_o[1] = SIN_COS_POLY4_U ? nl_intermediate_0_q : nl_intermediate_1_q;
                    addmul_rnd_mode_o    = fpnew_pkg::RNE;
                    addmul_op_o          = fpnew_pkg::MUL;
                    addmul_op_mod_o      = 1'b0;
                    addmul_tag_o         = addmul_res_q.tag;
                ...
                SIN_COS_POLY5_U, SIN_COS_POLY5_L: begin 
                    addmul_operands_o[0] =  SIN_COS_POLY5_U ? nl_intermediate_2_q : nl_intermediate_3_q;       
  ...
  always_comb begin : output_postproc
    ...
    else if ((op_i== COS) && (sin_cos_state_q == SIN_COS_DRAIN_L ||sin_cos_state_q == SIN_COS_DRAIN_U)) begin
      
      // Lane 1 (Upper 32 bits) - use pre-selected k_int
      unique case (k_int_sel[3:2])
        2'b00: {sign_1, x32_1} = {addmul_res_q.result[63],         addmul_res_q.result[63:32]};
        2'b01: {sign_1, x32_1} = {nl_intermediate_sel[63] ^ 1'b1,      nl_intermediate_sel[63:32]};
        2'b10: {sign_1, x32_1} = {addmul_res_q.result[63] ^ 1'b1,  addmul_res_q.result[63:32]};
        2'b11: {sign_1, x32_1} = {nl_intermediate_sel[63],             nl_intermediate_sel[63:32]};
      endcase
      
      // Lane 0 (Lower 32 bits)
      unique case (k_int_sel[1:0])
        2'b00: {sign_0, x32_0} = {addmul_res_q.result[31],        addmul_res_q.result[31:0]};
        2'b01: {sign_0, x32_0} = {nl_intermediate_sel[31] ^ 1'b1,     nl_intermediate_sel[31:0]};
        2'b10: {sign_0, x32_0} = {addmul_res_q.result[31] ^ 1'b1, addmul_res_q.result[31:0]};
    ...
    // --- SINE Reconstruction ---
    else if ((op_i== SIN) && (sin_cos_state_q == SIN_COS_DRAIN_L ||sin_cos_state_q == SIN_COS_DRAIN_U)) begin
      
      // Lane 1 (Upper 32 bits)
      unique case (k_int_sel[3:2])
        2'b00: {sign_1, x32_1} = {nl_intermediate_sel[63],            nl_intermediate_sel[63:32]};
        2'b01: {sign_1, x32_1} = {addmul_res_q.result[63],        addmul_res_q.result[63:32]};
        2'b10: {sign_1, x32_1} = {nl_intermediate_sel[63] ^ 1'b1,     nl_intermediate_sel[63:32]};
        2'b11: {sign_1, x32_1} = {addmul_res_q.result[63] ^ 1'b1, addmul_res_q.result[63:32]};
      endcase
      
      // Lane 0 (Lower 32 bits)
      unique case (k_int_sel[1:0])
        2'b00: {sign_0, x32_0} = {nl_intermediate_sel[31],            nl_intermediate_sel[31:0]};
        2'b01: {sign_0, x32_0} = {addmul_res_q.result[31],        addmul_res_q.result[31:0]};
        2'b10: {sign_0, x32_0} = {nl_intermediate_sel[31] ^ 1'b1,     nl_intermediate_sel[31:0]};
        2'b11: {sign_0, x32_0} = {addmul_res_q.result[31] ^ 1'b1, addmul_res_q.result[31:0]};
      endcase
  ...
  always_comb begin : handshake_management
    ...
    reconstructed_result_valid_o = (((tanh_state_q == TANH_DRAIN_U || tanh_state_q == TANH_DRAIN_L)) ||
                                    (sin_cos_state_q == SIN_COS_DRAIN_U ||sin_cos_state_q == SIN_COS_DRAIN_L))&& addmul_out_valid_i;
    ...
    if (nl_active) begin
      unique case (op_i) 
        ...
        SIN,COS: begin
          opgrp_out_ready_o[0] = addmul_out_ready_str ;
          opgrp_out_ready_o[3] = conv_out_ready_str;
          addmul_res_ready_d   = addmul_out_valid_i;
          conv_res_ready_d     = conv_out_valid_i;
          arb_req_o[3]         = 1'b0;
          arb_req_o[0]         = (sin_cos_state_q == SIN_COS_DRAIN_L || sin_cos_state_q == SIN_COS_DRAIN_U) && addmul_out_valid_i; 
        end
