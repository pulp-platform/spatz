...
`endif
  ...
  localparam int unsigned NrWritePorts = 2 + NrInterfaces; // 1 for VFU and SLDU each and 1 for each VLSU
  localparam int unsigned NrReadPorts  = 4 + 2*NrInterfaces; // 3 for VFU, 1 for SLDU and 2 for each VLSU interface
  ...
  logic vrf_buf_en, vlsu_buf_en;
  logic vrf_vfu_wvalid, vrf_vlsu_wvalid;

  logic buf_full, buf_empty, vlsu_buf_full, vlsu_buf_empty;
...
`ifdef TROOP
  spatz_troop_vlsu #(
    ...
    .vrf_id_o                ({sb_id[SB_VLSU_VD_WD1], sb_id[VLSU_VD_RD1], sb_id[VLSU_VD_RD0],     // VLSU Interface-1
                               sb_id[SB_VLSU_VD_WD0], sb_id[VLSU_VS2_RD1], sb_id[VLSU_VS2_RD0]}), // VLSU Interface-0
    // Interface Memory
