...
`endif
  ...
  for (genvar intf = 0; intf < NrInterfaces; intf++) begin : gen_vrf_req_register_intf
    ...
    bitwise_TMR_voter_fail #(
      .DataWidth (1),
      .VoterType (1)
    ) i_vrf_commit_pending_voter (
      .a_i              (vrf_commit_pending_q_rep[0]        ),
      .b_i              (vrf_commit_pending_q_rep[1]        ),
      .c_i              (vrf_commit_pending_q_rep[2]        ),
      .majority_o       (vrf_commit_pending_q[intf]         ),
