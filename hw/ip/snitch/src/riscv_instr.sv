...
package riscv_instr;
  ...
  // Custom scalar FP load + RRPOST (rs1 <- rs1 + rs2), custom-0 opcode
  // R-type-like custom format:
  //   [31:25] funct7 (custom tag)
  //   [24:20] rs2    (increment GPR)
  //   [19:15] rs1    (base GPR)
  //   [14:12] width  (matches FL* width)
  //   [11:7]  rd     (FP destination reg field)
  //   [6:0]   opcode = custom-0 (0101011)
  localparam logic [31:0] P_FLB_RRPOST       = 32'b1010000??????????000?????0101011;
  localparam logic [31:0] P_FLH_RRPOST       = 32'b1010000??????????001?????0101011;
  localparam logic [31:0] P_FLW_RRPOST       = 32'b1010000??????????010?????0101011;
  localparam logic [31:0] P_FLD_RRPOST       = 32'b1010000??????????011?????0101011;
  ...
  localparam logic [31:0] P_VLE8_V_RRPOST    = 32'b???100???????????000?????0000111;
  localparam logic [31:0] P_VLE16_V_RRPOST   = 32'b???100???????????101?????0000111;
  localparam logic [31:0] P_VLE32_V_RRPOST   = 32'b???100???????????110?????0000111;
  localparam logic [31:0] P_VLE64_V_RRPOST   = 32'b???100???????????111?????0000111;
