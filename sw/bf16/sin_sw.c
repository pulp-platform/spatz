/*  sin_sw.c – Cody-Waite + polynomial SIN for BF16
 *
 *  Algorithm: quadrant reduction, SIN blend (sign_s = 1 - 2*b1)
...
static inline void vsin_m8(const uint16_t *inp, uint16_t *out, int N) {
    ...
    while (rem > 0) {
        ...
        /* SIN blend: sign_s = 1 - 2*b1, sin = s0 + b0*(c0 - s0) */
        asm volatile(
            "vle16.v    v0,  (%[kbuf])          \n\t"
            "vand.vi    v24, v0,  1             \n\t"  /* b0 = k & 1 */
            "vsrl.vi    v0,  v0,  1             \n\t"
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        check_bf16(g_out, outS, N, "SIN_SW");
    }
