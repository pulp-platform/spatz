/*  rsqrt_sw.c – Software inverse-sqrt via Quake magic + 2 NR iterations (FP16)
 *
 *  Algorithm:
 *    y = reinterpret_fp16( MAGIC - (reinterpret_int16(a) >> 1) )
 *    half = a * 0.5
 *    2× NR:  t = y*y;  t = half*t;  t = 1.5 - t;  y = y*t
 *
 *  MAGIC for FP16: 0x59B8   (analogous to 0x5F37 for BF16, 0x5F3759DF for FP32)
 *
...
static const unsigned int QUAKE_MAGIC = 0x59B8u;
#define FP16_HALF      0x3800u   /* 0.5 */
#define FP16_ONE_PT_5  0x3E00u   /* 1.5 */

/* ========================= LMUL = 8 ========================= */
static inline void vrsqrt_sw_m8(const uint16_t *inp, uint16_t *out, int N) {
    float half = bf16_scalar(FP16_HALF);
    float one_point_five = bf16_scalar(FP16_ONE_PT_5);
    int rem = N;
    while (rem > 0) {
        size_t vl;
        ...
        asm volatile(
            ...
            "vfmul.vv   v16, v16, v8             \n\t"
            : : [pin]"r"(inp),
                [M]"r"(QUAKE_MAGIC), [h]"f"(half), [p]"f"(one_point_five)
            : "memory"
        );
        /* reload a for second NR (overwritten above) */
        asm volatile(
            "vle16.v    v8,  (%[pin])           \n\t"
            "vfmul.vf   v24, v8,  %[h]          \n\t"
            /* NR iteration 2 */
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RSQRT_FP16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outR, N, "RSQRT_FP16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
