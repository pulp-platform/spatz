/*  cos_sw.c – Software cosine via Cody-Waite range reduction + cheap polynomial (FP16)
 *
 *  Algorithm:
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[COS_FP16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outCos, N, "COS_FP16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
