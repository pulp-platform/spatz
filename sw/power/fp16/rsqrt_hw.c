/*  rsqrt_hw.c - Hardware inverse-sqrt via vfrsqrt7.v burst pattern (FP16)
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RSQRT_HW_FP16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outR, N, "RSQRT_HW_FP16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
