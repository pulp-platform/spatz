/*  rec_hw.c – Hardware reciprocal via vfrec7.v burst pattern
 *
 *  Algorithm:  4-way unrolled load → vfrec7.v → store
 *  The HW instruction provides 7-bit accurate reciprocal seed.
 *
...
/* Burst unroll factor — 4 groups loaded/computed/stored per iteration. */

/* ========================= LMUL = 8 =========================
 * Groups: v0, v8, v16, v24   (4 groups of 8 = 32 regs)
 */
static inline void vrec_hw_m8(const float *inp, float *out, int N) {
    size_t vl;
    asm volatile("vsetvli %0, zero, e32, m8, ta, ma" : "=r"(vl));
    int iters = N / (int)(4u * vl);
    for (int i = 0; i < iters; i++) {
        asm volatile(
            "vle32.v  v0,  (%0)" :: "r"(inp)            : "memory");
        asm volatile(
            "vle32.v  v8,  (%0)" :: "r"(inp +     vl)   : "memory");
        asm volatile(
            "vle32.v  v16, (%0)" :: "r"(inp + 2u*vl)    : "memory");
        asm volatile(
            "vle32.v  v24, (%0)" :: "r"(inp + 3u*vl)    : "memory");
        asm volatile("" ::: "memory");
        ...
        asm volatile(
            "vse32.v  v0,  (%0)" :: "r"(out)            : "memory");
        asm volatile(
            "vse32.v  v8,  (%0)" :: "r"(out +     vl)   : "memory");
        asm volatile(
            "vse32.v  v16, (%0)" :: "r"(out + 2u*vl)    : "memory");
        asm volatile(
            "vse32.v  v24, (%0)" :: "r"(out + 3u*vl)    : "memory");
        inp += 4u * vl;
        out += 4u * vl;
    }
    /* remainder (strip-mined) */
...
/* ========================= LMUL = 4 ========================= */
static inline void vrec_hw_m4(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfrec7.v v0,  v0");
        asm volatile("vfrec7.v v4,  v4");
        asm volatile("vfrec7.v v8,  v8");
        asm volatile("vfrec7.v v12, v12");
        asm volatile("vse32.v v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 2 ========================= */
static inline void vrec_hw_m2(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfrec7.v v0,  v0");
        asm volatile("vfrec7.v v2,  v2");
        asm volatile("vfrec7.v v4,  v4");
        asm volatile("vfrec7.v v6,  v6");
        asm volatile("vse32.v v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 1 ========================= */
static inline void vrec_hw_m1(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfrec7.v v0, v0");
        asm volatile("vfrec7.v v1, v1");
        asm volatile("vfrec7.v v2, v2");
        asm volatile("vfrec7.v v3, v3");
        asm volatile("vse32.v v0, (%0)" :: "r"(out)           : "memory");
...
static inline void vrec_hw_kernel(const float *inp, float *out, int N) {
#if   LMUL_MODE == 8
    vrec_hw_m8(inp, out, N);
#elif LMUL_MODE == 4
    vrec_hw_m4(inp, out, N);
#elif LMUL_MODE == 2
    vrec_hw_m2(inp, out, N);
#elif LMUL_MODE == 1
    vrec_hw_m1(inp, out, N);
#else
...
int main(void) {
    ...
    if (len > 0) vrec_hw_kernel(g_inp + start, g_out + start, len);
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[REC_HW LMUL=%d] cycles=%u  cores=%u  N=%d\n",
