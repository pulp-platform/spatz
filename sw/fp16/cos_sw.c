/*  cos_sw.c – Software cosine via Cody-Waite + polynomial (FP16)
 *
 *  Algorithm:
 *    r = x - round(x * 2/pi) * pi/2
 *    z = r*r
 *    cos_poly = 1 + C2*z,   sin_poly = r*(1 + S3*z)
 *    Maskless quadrant blend
 *
...
static const float C2      = -0.49609375f;
static const float S3      = -0.16601562f;
static const float INVPIO2 =  0.63671875f;
static const float PIO2    =  1.5703125f;
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        check_fp16(g_out, outCos, N, "COS_SW");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
