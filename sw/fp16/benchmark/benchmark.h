 ...
 * Converts FP16 bit patterns to float, uses multiply-based comparison
...
static inline float fp16_to_float(uint16_t h) {
    ...
    float f;
    __builtin_memcpy(&f, &f32, sizeof(f));
    return f;
}

static inline float fabsf_inline(float x) { return x < 0.0f ? -x : x; }

static inline int check_fp16(const uint16_t *got, const uint16_t *ref, int n,
                             const char *tag) {
    enum { MAX_PRINT = 5 };
    const float TOL = 0.03f;                 /* 3 % relative tolerance */
    int errs = 0;
    for (int i = 0; i < n; i++) {
        float g = fp16_to_float(got[i]);
        float r = fp16_to_float(ref[i]);
