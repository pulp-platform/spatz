...
#include DATAHEADER
#include "kernel/hp-alt-fmatmul.c"
...
int verify_matrix(uint16_t *matrix, uint16_t *checksum,
                  const unsigned int num_rows, const unsigned int num_columns) {
  for (unsigned int i = 0; i < num_rows; ++i) {
    for (unsigned int j = 0; j < num_columns; ++j) {
      float computed = bf16_to_fp32(&matrix[i * num_columns + j]);
      float ref = bf16_to_fp32(&checksum[i * num_columns + j]);
      float abs_error =
          (computed - ref) > 0 ? (computed - ref) : (ref - computed);
      float abs_ref = ref > 0 ? ref : -ref;
      if ((abs_error > 0.05f * abs_ref) && (abs_error > 0.5f)) {
