...
def array_to_cstr(a, fmt=float):
    ...
    if fmt == float:
        ...
        a_uint16 = a.view(torch.int16).numpy().view(np.uint16).flat
        for el in a_uint16:
            out += f"0x{el:04x}, "
    else:
...
def emit_GEMM_layer(name="gemm", **kwargs):
    ...
    if kwargs["prec"] == 16:
        ...
        layer_str += (
            f'static uint16_t {name}_A_dram [{m}*{k}] __attribute__((section(".data"))) = '
            + array_to_cstr(mat_A, fmt="uint16_t")
            + ";\n\n\n"
        )
        layer_str += (
            f'static uint16_t {name}_B_dram [{k}*{n}] __attribute__((section(".data"))) = '
            + array_to_cstr(mat_B, fmt="uint16_t")
            + ";\n\n\n"
        )
        layer_str += (
            f'static uint16_t {name}_C_dram [{m}*{n}] __attribute__((section(".data"))) = '
            + array_to_cstr(mat_C, fmt="uint16_t")
            + ";\n\n\n"
        )
        layer_str += (
            f'static uint16_t {name}_checksum[{m}*{n}] __attribute__((section(".data"))) = '
            + array_to_cstr(result, fmt="uint16_t")
            + ";\n\n\n"
        )
    elif dtype != "char":
        layer_str += (
...
def rand_data_generator(shape, prec, alt=True):
    assert prec == 16 and alt, "hardcoding alt=True for BF16 data"
    if prec == 64:
