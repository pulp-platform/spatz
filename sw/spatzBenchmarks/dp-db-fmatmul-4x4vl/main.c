...
// This is dimensions can be fit in 128 KiB SPM
// #define matrix_M 16
// #define matrix_K 
// #define matrix_N 32

// Doing matmul on 128x128x128, need to be done in 16 loops
// Each loops need A: 2x16x128, B: 2x128x32
// Producing C: 2x16x32
// Each core works on 2(8x128 * 128x32) => 2(8x32) => 16x32
// 2(16x128 + 128x32 + 16x32) * 8/1024 = 104 KiB
...
int main() {
  ...
  for (int out_loop = 0; out_loop < tot_out; out_loop++) {
    ...
    if (cid == 0 & (out_loop < tot_out-1)) {
      ...
      // snrt_dma_start_1d(b_dma, b_l2, B_size * sizeof(double));
      // We want to load a K-by-N matrix to SPM, each stride has length of N
      // then it jumps to next row repeating K times
    ...
    a_l2    = gemm_A_dram;
    c_l2    = gemm_C_dram + gemm_l.N * (out_loop+1);
    c_l2_st = gemm_C_dram + gemm_l.N * (out_loop+1);
  }
