...
int main() {
  ...
  // Allocate the matrices
    ...
    // 1. Allocate the output array
    if (cid == 0) {
      result    = result_buf_dram;
    ...
    // 2. Setup core-specific pointers
    T *mat_core    = gemv_mat_dram + m_core * cid;
    T *vec_core    = gemv_vec_dram;
    ...
    if (cid == 0) {
      for (unsigned int j = 0; j < gemv_l.N; ++j) {
        if ((double) vec_core[j] != 0.0) {
          nz_count++;
        }

        if (nz_count == tot_nz_dram) break;
      }
    }

  #ifdef FINDNZ
    // 3. Zero-initialize the accumulator memory
    for (unsigned int i = 0; i < m_core; i++) {
      result_core[i] = 0.0;
    }
    snrt_cluster_hw_barrier();
    ...
    // 4. Combined Search & Compute Loop
    nz_count = 0;

    if (cid == 0) {
      for (unsigned int j = 0; j < gemv_l.N; ++j) {
        if ((double) vec_core[j] != 0.0) {
          dense_vec[nz_count] = vec_core[j];
    ...
    snrt_cluster_hw_barrier();
    timer_nz = benchmark_get_cycle() - timer;
    timer = benchmark_get_cycle();

    // Each core calculates its offset base pointer
    T *mat_core_base = gemv_mat_dram + m_core * cid;

    // One single call per core. The kernel handles the scattered column fetching!
    #if (PREC == 64)
    sa_gemv_cache_v64b_m4(mat_core_base, dense_vec, dense_idx, result_core, gemv_l.M, m_core, tot_nz_dram);
    #elif (PREC == 32)
    sa_gemv_cache_v32b_m4(mat_core_base, dense_vec, dense_idx, result_core, gemv_l.M, m_core, tot_nz_dram);
    #else
    sa_gemv_cache_v16b_m4(mat_core_base, dense_vec, dense_idx, result_core, gemv_l.M, m_core, tot_nz_dram);
    #endif
    ...
    for (unsigned int j = 0; j < gemv_l.N; j++) {
      if ((double)vec_core[j] != 0.0) {
        // Pass N=1 because we are computing exactly 1 column for this non-zero.
        // The matrix pointer is offset by 'j * gemv_l.M' to fetch the correct column.
    #if (PREC == 64)
        gemv_v64b_m4(mat_core + j * gemv_l.M, &vec_core[j], result_core, gemv_l.M, m_core, 1);
    #elif (PREC == 32)
        gemv_v32b_m4(mat_core + j * gemv_l.M, &vec_core[j], result_core, gemv_l.M, m_core, 1);
    #else
        gemv_v16b_m4(mat_core + j * gemv_l.M, &vec_core[j], result_core, gemv_l.M, m_core, 1);
    #endif
        nz_count++;
        if (nz_count == tot_nz_dram) break;
      }
