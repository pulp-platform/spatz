      ...
      if (iter == 0) {
        ...
        for (unsigned int i = 0; i < NFFT; i++) {
          if (fp_check(buffer[i], gold_out_dram[2 * i])) {
            #ifdef PRINT_RESULT
            printf("Error: Index %d -> Result = %f, Expected = %f, buffer[%d] addr = %p\n", i,
                   (float)buffer[i], (float)gold_out_dram[2 * i], i + NFFT, &buffer[i]);
            #endif
          }
        }
        ...
        for (unsigned int i = 0; i < NFFT; i++) {
          if (fp_check(buffer[i + NFFT], gold_out_dram[2 * i + 1])) {
            #ifdef PRINT_RESULT
            printf("Error: Index %d -> Result = %f, Expected = %f, buffer[%d] addr = %p\n", i + NFFT,
                   (float)buffer[i + NFFT], (float)gold_out_dram[2 * i + 1], i + NFFT, &buffer[i]);
            #endif
          }
        }
      }
