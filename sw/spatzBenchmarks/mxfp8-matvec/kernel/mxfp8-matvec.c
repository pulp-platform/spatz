...
// - 4x data reuse
void mxfp8_matvec_fp32_inner_4x(float *c, const char *a, const char *b,
  ...
  for (uint32_t m = 0; m < M; m += 4) {
    ...
    for (uint32_t n = 0; n < N; n += MXFP8_BLOCK_SIZE) {
      ...
      asm volatile(
          "vsetvli zero, %0, e32, m1, ta, ma" ::"r"(MXFP8_BLOCK_SIZE / 2));

      // load scales, add and re-bias to FP32
      const uint8_t *a_scale_ =
          (const uint8_t *)a_scale + m * N_BLOCK + n_block;
...
//   block) fit into a vector register group with LMUL=2
// - 4x data reuse
void mxfp8_matvec_fp32_inner_sdotp_4x(float *c, const char *a, const char *b,
                                      const char *a_scale, const char *b_scale,
                                      const uint32_t M, const uint32_t N) {
  uint32_t N_BLOCK = N / MXFP8_BLOCK_SIZE;
  ...
  for (uint32_t m = 0; m < M; m += 4) {
    ...
    for (uint32_t n = 0; n < N; n += MXFP8_BLOCK_SIZE) {
      ...
      asm volatile(
          "vsetvli zero, %0, e32, m4, ta, ma" ::"r"(2 * MXFP8_BLOCK_SIZE));
      ...
      // widen and multiply operands to FP32 using ExSdotp
      asm volatile("vsetvli zero, %0, e16, m1, ta, ma" ::"r"(MXFP8_BLOCK_SIZE));
      asm volatile("vfwdotp.vv  v8, v16, v20");
      asm volatile("vfwdotp.vv  v9, v17, v20");
      asm volatile("vfwdotp.vv v10, v18, v20");
      asm volatile("vfwdotp.vv v11, v19, v20");
      ...
      if (n == 0) {
        asm volatile("vfmul.vf v0,  v8, %0" ::"f"(scale0));
        asm volatile("vfmul.vf v2,  v9, %0" ::"f"(scale1));
        asm volatile("vfmul.vf v4, v10, %0" ::"f"(scale2));
        asm volatile("vfmul.vf v6, v11, %0" ::"f"(scale3));
      } else {
        asm volatile("vfmacc.vf v0, %0,  v8" ::"f"(scale0));
        asm volatile("vfmacc.vf v2, %0,  v9" ::"f"(scale1));
        asm volatile("vfmacc.vf v4, %0, v10" ::"f"(scale2));
        asm volatile("vfmacc.vf v6, %0, v11" ::"f"(scale3));
      }
    }
