...
def emit_GEMM_layer(name='gemm', **kwargs):
    ...
    layer_str += f'static {dtype} {name}_results [{m}*{n}] = ' + array_to_cstr(kwargs['result'], kwargs['prec']) + ';\n\n\n'
...
def rand_data_generator(shape, prec, alt=False):
    if prec == 64:
        ...
        return torch.randint(low=0, high=999, size=shape, requires_grad=False, dtype=torch.int32), {}
    elif prec == 16:
