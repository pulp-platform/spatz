...
def emit_jacobi2d_layer(name='jacobi2d', **kwargs):
    ...
    if dtype != 'char':
        layer_str += f'static {dtype} {name}_A_dram [{r}*{c}] __attribute__((section(".data"))) = ' + array_to_cstr(vec_A) + ';\n\n\n'
        layer_str += f'static {dtype} {name}_B_dram [{r}*{c}] __attribute__((section(".data"))) = ' + array_to_cstr(vec_B) + ';\n\n\n'
        layer_str += f'static {dtype} {name}_result [{r}*{c}] __attribute__((section(".data"))) = ' + array_to_cstr(result) + ';\n\n\n'
    else:
        layer_str += f'static {dtype} {name}_A_dram [{r}*{c}] = ' + \
            array_to_cstr(kwargs['bits_A'], fmt='char') + ';\n\n\n'
        layer_str += f'static {dtype} {name}_B_dram [{r}*{c}] = ' + \
            array_to_cstr(kwargs['bits_B'], fmt='char') + ';\n\n\n'
        layer_str += f'static {dtype} {name}_result [{r}*{c}] = ' + \
...
def jacobi2d(a, b, temp, R, C):
    ...
    for i in range(1, R-1):
        for j in range(1, C-1):
            temp[i][j] = 0.2 * (b[i][j] + b[i-1][j] + b[i+1][j] + b[i][j-1] + b[i][j+1]);
    return temp;
