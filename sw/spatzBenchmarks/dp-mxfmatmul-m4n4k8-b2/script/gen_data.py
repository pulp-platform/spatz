...
def emit_header_file(layer_type: str, **kwargs):
    ...
    elif layer_type == 'Print':
        file = file_path / 'result_check.h'
        emit_str += emit_Print_layer(**kwargs)
...
def main():
    ...
    if param['kernel'] == 'Conv2d':
        ...
        print("Mat_A Original:")
        for row in mat_A:
            print(" ".join(str(struct.unpack('i', struct.pack('f', x))[0]) for x in row))

        print("Mat_B Original:")
        for row in mat_B:
            print(" ".join(str(struct.unpack('i', struct.pack('f', x))[0]) for x in row))
        ...
        emit_header_file('Print', mat=result)
