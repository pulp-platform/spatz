...
// MXFP8 dot product
// - block size = 32 fixed
// - avl == LENGTH(a) == LENGTH(b) == 32 * LENGTH(a_scale) == 32 *
// LENGTH(b_scale)
...
float mxfp8_dotp_fp32(const char *a, const char *b, const char *a_scale,
  ...
  do {
    for (unsigned int i = 0; i < MXFP8_BLOCK_SIZE; i++) {
      asm volatile("vsetvli %0, %1, e8, m1, ta, ma"
                   : "=r"(block_vl)
                   : "r"(block_avl));
      asm volatile("vlse8.v v8, (%0), %1" ::"r"(a + i),
                   "r"(MXFP8_BLOCK_SIZE * sizeof(char)));
      asm volatile("vlse8.v v9, (%0), %1" ::"r"(b + i),
                   "r"(MXFP8_BLOCK_SIZE * sizeof(char)));
