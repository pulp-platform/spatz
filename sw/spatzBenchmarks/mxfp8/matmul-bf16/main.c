  ...
  for (uint32_t i = 0; i < m; i++) {
    for (uint32_t j = 0; j < n; j++) {
      ...
      if (diff > eps) {
        ...
        printf(" acutal   = 0x%04x = %.4f\n", act_int, act);
        printf(" expected = 0x%04x = %.4f\n", exp_int, exp);
        return false;
      }
    }
  }
...
#ifdef MXDOTP
    mxfp8_matmul_bf16_mxdotp_lmul1_8x(local_c, local_a, local_b, local_a_scale,
...
#elif BASELINE_MIN
    mxfp8_matmul_bf16_min_lmul2_4x(local_c, local_a, local_b, local_a_scale,
                                   local_b_scale, local_m, local_n, local_k);
#else
    mxfp8_matmul_bf16_outer_lmul2_4x(local_c, local_a, local_b, local_a_scale,
                                     local_b_scale, local_m, local_n, local_k);
#endif
