...
#else
  asm volatile("vmfgt.vf v1, v2, %[A]" ::[A] "f"(fscalar_32));
#endif;
  VSET(1, e16, m1);
...
void TEST_CASE2(void) {
  ...
  VCLEAR(v8);
#if ELEN == 64
  asm volatile("vmfgt.vf v8, v2, %[A], v0.t" ::[A] "f"(dscalar_32));
#else
  asm volatile("vmfgt.vf v8, v2, %[A], v0.t" ::[A] "f"(fscalar_32));
#endif
  VSET(1, e16, m1);
  VCMP_U16(5, v8, 0x0008);
#if ELEN == 64
  VSET(8, e64, m2);
  double dscalar_64;
  //                               -0.3394093097660049
  ...
  VLOAD_8(v0, 0xAA, 0xAA);
  VCLEAR(v8);
  asm volatile("vmfgt.vf v8, v2, %[A], v0.t" ::[A] "f"(dscalar_64));
  VSET(1, e8, m1);
  VCMP_U8(6, v8, 0x22);
#endif
};
