 ...
 * -----------------------------------------------------------------------
 *
 * RVV rsqrtf (SEW=16) — strip-mined, maskless, runtime-selectable LMUL (8/4/2).
 *
 * Algorithm
...
/* FP16 Quake-style rsqrt magic: adapted from FP32 0x5F3759DF for 16-bit format */
#ifndef RSQRT_MAGIC
#define RSQRT_MAGIC 0x59B8u
#define HALF        0.5f
...
/* ========================= LMUL = 8 (3 regs used) =========================
 * Reg plan (SEW=16, LMUL=8):
 *   v8  : x (__fp16)
 *   v16 : y (__fp16)   [seed -> refined result]
 ...
 *   y  = y * t         -> v16
 */
static inline void vrsqrt_m8_strip(const __fp16* inp, __fp16* out, int N) {
    ...
    while (remaining > 0) {
        ...
        /* v0 = 1.5 - 0.5*v24 (use v0 as temp since LMUL=8 only allows v0,v8,v16,v24) */
        asm volatile("vfmv.v.f  v0, %[c1]":: [c1]"f"(ONE_POINT_FIVE));
        asm volatile("vfnmsac.vf v0, %[c05], v24" :: [c05]"f"(HALF));
        asm volatile("vfmul.vv  v16, v16, v0");                       /* y = y * v0 */
 ...
 */
void vrsqrt_sw(const __fp16* inp, __fp16* out, int N) {
    const __fp16 *pin  = inp;
    __fp16       *pout = out;
    int remaining = N;
    ...
    while (remaining > 0) {
        ...
        asm volatile("vsub.vv   v16, v24, v16");       // v16 = MAGIC - (x >> 1)
...
void vrsqrt_optimized(const __fp16* inp, __fp16* out,  int N) {
    const __fp16 *pin = inp;
    __fp16 *pout = out;
    int remaining = N;
    // N = 2048
...
/* ========================= LMUL = 4 (baseline) ========================= */
static inline void vrsqrt_m4_strip(const __fp16* inp, __fp16* out, int N) {
    const __fp16 *pin  = inp;
    __fp16       *pout = out;
    int remaining = N;
...
/* ========================= LMUL = 2 (more regs) ========================= */
static inline void vrsqrt_m2_strip(const __fp16* inp, __fp16* out, int N) {
    const __fp16 *pin  = inp;
    __fp16       *pout = out;
    int remaining = N;
...
/* ------------------- Kernel selector ------------------- */
static inline void vrsqrt_strip(const __fp16* inp, __fp16* out, int N) {
#if   (LMUL_MODE == 8)
