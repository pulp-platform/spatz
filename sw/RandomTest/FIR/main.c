...
void FIR_v4 (int16_t *Signal, float *Filter, int16_t *Output,int FilterLength, int InputLength, unsigned int vl){
    int itr = 0;
    int num_elements = InputLength;
    int VLMAX = 16;
    
    asm volatile("vsetvli  %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(VLMAX));
    while(itr*VLMAX + VLMAX <= num_elements){
      for (int i = 0; i < FilterLength; i++){
        asm volatile("vle16.v v0, (%0)" :: "r"(Signal+itr*VLMAX+i));
        asm volatile("vse16.v v0, (%0)" :: "r"(Output+itr*VLMAX));
      }
      itr++;
    }
}
...
int main() {
    ...
    if (cid == 0) {
        input = (int16_t   *)snrt_l1alloc(LENGTH * sizeof(int16_t));
        snrt_dma_start_1d(input, input_dram, LENGTH * sizeof(int16_t));
        ...
        output = (int16_t   *)snrt_l1alloc(LENGTH * sizeof(int16_t));
