/*  RMSNorm baseline – Root Mean Square Normalization using SW rsqrt (RV64GCV)
 *
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        conv.f = mean_sq;
        conv.u = RSQRT_MAGIC - (conv.u >> 1);
        float y = conv.f;
        float half_x = 0.5f * mean_sq;
        y = y * (1.5f - half_x * y * y);   /* NR iter 1 */
        y = y * (1.5f - half_x * y * y);   /* NR iter 2 */
        *g_inv_rms = y;
    }
    snrt_cluster_hw_barrier();
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RMSNORM 2-CORE] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
