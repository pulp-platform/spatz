 ...
 *  ~14 bits (~4 decimal digits), close to full BF16 mantissa precision
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        { float nr_threehalves = h_scalar(0x3FC0u);
        float nr_half = h_scalar(0x3F00u);
        asm volatile(
            "vsetivli zero, 1, e16, m1, ta, ma \n\t"
            "vfmv.v.f v0, %[ms] \n\t"
            "vfmul.vf v0, v0, %[hx] \n\t"
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RMSNORM CUSTOM 2-CORE BF16] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
