/*  Stable Softmax baseline – 2-core parallel (RV64GCV / Snitch-Spatz)
 *
 *  Kernel:  softmax_i = exp(x_i - max) / Σ exp(x_j - max)
 *
 *  Parallelisation strategy (shared L1, hardware barriers):
 *    Pass 1: each core finds local max via vfredmax.vs
 *             → barrier → core 0 reduces partial maxes → global max
 *    Pass 2: each core computes exp(x-max) via Schraudolph and
 *             accumulates local sum via vfredusum.vs
 *             → barrier → core 0 reduces partial sums → global sum
 *    Pass 3: core 0 computes inv_sum (Newton-Raphson)
 *             → barrier → each core multiplies its chunk by inv_sum
 *
 *  NUM_CORES is set to 2 but the code generalises to any core count
 *  by changing the constant (and the allocation size for partials).
 *
 *  LMUL = 4 (e32)
 */
...
/* Number of cores to use for softmax (≤ cluster core count) */
#define SM_CORES 2
...
static float *g_inv_sum;       /* 1 float – broadcast to all cores */

/* -----------------------------------------------------------------------
 * pass1_local_max – vectorised max reduction over a contiguous chunk.
 * Writes the chunk maximum into partial_max[cid].
 * ----------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------
 * pass2_exp_and_sum – Schraudolph exp(x-max) + vfredusum local sum.
 * Stores exp results into exp_buf and partial sum into partial_sum[cid].
 * ----------------------------------------------------------------------- */
static void pass2_exp_and_sum(const float *src, float *dst,
    ...
    while (rem > 0) {
        ...
        asm volatile(
            ...
            /* Schraudolph exp – bitcast via typeless registers */
            "vfmv.v.f   v4,  %[B]                 \n\t"
            "vfmacc.vf  v4,  %[C], v0              \n\t"
...
int main(void) {
    ...
    /* Only the first SM_CORES cores participate */
    if (cid >= SM_CORES) {
        /* idle cores just hit every barrier and exit */
    ...
    if (cid == 0) {
        ...
        /* Broadcast via shared memory so all active cores can read it.
         * We reuse g_partial_max[0] as the broadcast slot. */
        g_partial_max[0] = global_max;
    }
    snrt_cluster_hw_barrier();
    global_max = g_partial_max[0];   /* all cores read broadcast value */

    /* ==================================================================
     * Pass 2 — parallel exp + local sum, then core 0 reduces
     * ================================================================== */
    if (len > 0)
     ...
     * ================================================================== */
    float inv = *g_inv_sum;   /* all cores read broadcast value */
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[SOFTMAX 2-CORE] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
