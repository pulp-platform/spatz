 ...
 * -----------------------------------------------------------------------
 * RVV sincos (SEW=16) — strip-mined, maskless, runtime-selectable LMUL.
 *
 * Two kernels:
...
static __fp16 *g_inp  = NULL;
static __fp16 *g_outc = NULL;
static __fp16 *g_outs = NULL;

// Polynomial and reduction constants (match golden reference)
static inline void sincos_consts(__fp16 *INVPIO2, __fp16 *PIO2_HI, __fp16 *PIO2_LO,
                                 __fp16 *S1, __fp16 *S3, __fp16 *S5, __fp16 *S7,
                                 __fp16 *C2, __fp16 *C4, __fp16 *C6) {
...
static inline void vcos_f32_m8_strip(const __fp16* inp, __fp16* outc, int N) {
    __fp16 INVPIO2, PIO2_HI, PIO2_LO, S1, S3, S5, S7, C2, C4, C6;
    ...
    while (remaining > 0) {
        ...
        /* sign_c = 1 - 2*(b1^b0)  (int -> __fp16 in v16) */
        ...
        asm volatile("vse16.v v24, (%0)" :: "r"(pout) : "memory");
...
static inline void vcos_f32_sw(const __fp16* inp, __fp16* outc, int N) {
    const __fp16 C2 = -0.49670f; 
    const __fp16 S3 = -0.16605f; 
    const __fp16 INVPIO2 = 0.63661977f;
    const __fp16 PIO2 = 1.57079632679489661923f;  // Single high-precision constant
    ...
    while (remaining > 0) {
        ...
        asm volatile("vfcvt.f.x.v v0, v0");     // Convert to __fp16         
        // Convert b0 to __fp16         asm volatile("vfcvt.f.x.v v20, v20");
...
static inline void vcos_f32_m8_sw(const __fp16* inp, __fp16* outc, int N) {
    const __fp16 C2 = -0.49670f; 
    const __fp16 S3 = -0.16605f; 
    const __fp16 INVPIO2 = 0.63661977f;
    const __fp16 PIO2 = 1.57079632679489661923f;  // Single high-precision constant
    ...
    while (remaining > 0) {
        ...
        asm volatile("vle16.v v0, (%0)" :: "r"(pin) : "memory");  // Reload x to v0-v7
        ...
        asm volatile("vrsub.vi v0, v0, 1");    // v0 = 1 - 2*(b0^b1)
        asm volatile("vfcvt.f.x.v v0, v0");    // Convert to __fp16         
        // Convert b0 to __fp16         asm volatile("vfcvt.f.x.v v24, v24");  // v24 = b0 (float)
...
static inline void fast_sincos_cheapest_corrected_f32_m2(const __fp16* inp, __fp16* outs, __fp16* outc, int N) {
    const __fp16 C2 = -0.49670f; 
    const __fp16 S3 = -0.16605f; 
    const __fp16 INVPIO2 = 0.63661977f;
    const __fp16 PIO2_HI = 1.57079632f;
    const __fp16 PIO2_LO = 6.12323399e-17f; // Essential for phase stability

    while (N > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m2, ta, ma" : "=r"(vl) : "r"(N));

        asm volatile("vle16.v v4, (%0)" :: "r"(inp));
        ...
        asm volatile("vsll.vi v28, v28, 15");   // shift to FP16 sign bit
        asm volatile("vsll.vi v26, v26, 15");   // shift to FP16 sign bit
        asm volatile("vfsgnjx.vv v12, v12, v28"); // Apply to Cos
        asm volatile("vfsgnjx.vv v14, v14, v26"); // Apply to Sin

        asm volatile("vse16.v v14, (%0)" :: "r"(outs));
        asm volatile("vse16.v v12, (%0)" :: "r"(outc));
...
static inline void vfcos_hw_test(const __fp16* inp, __fp16* out, int N) {
  const __fp16 *pin  = inp;
  __fp16       *pout = out;
...
static inline void fast_sincos_cheapest_corrected_f32_m2_v1(
    const __fp16* inp, __fp16* outs, __fp16* outc, int N)
{
    const __fp16 C2      = -0.49670f;
    const __fp16 S3      = -0.16605f;
    const __fp16 INVPIO2 =  0.63661977f;
    const __fp16 PIO2_HI =  1.57079632f;
    const __fp16 PIO2_LO =  6.12323399e-17f;
    ...
    while (N > 0) {
        ...
        asm volatile("vse16.v v10, (%0)" :: "r"(outs));
        asm volatile("vse16.v v8,  (%0)" :: "r"(outc));
        inp  += vl; outs += vl; outc += vl; N -= vl;
    }
}
/* Fast single-output cosf with RVV (SEW=16, LMUL=4), strip-mined & maskless.
   Requires FRM = RNE so vfcvt.x.f.v does round-to-nearest-even for k. */

/* Fast single-output cosf with RVV (SEW=16, LMUL=4), strip-mined & maskless.
   Requires FRM = RNE so vfcvt.x.f.v does round-to-nearest-even for k. */

static inline void vcos_f32_m4_strip(const __fp16* inp, __fp16* outc, int N) {
    __fp16 INVPIO2, PIO2_HI, PIO2_LO, S1, S3, S5, S7, C2, C4, C6;
    sincos_consts(&INVPIO2, &PIO2_HI, &PIO2_LO, &S1, &S3, &S5, &S7, &C2, &C4, &C6);
...
/* ========================= LMUL = 4 =========================
 * Register plan (LMUL=4, aligned): *   v4   : y | kf | b0f | sign_c (__fp16) | temps
 *   v8   : k (int) | b0 (int)
 *   v12  : x | r | (1-b0f) | sign_s (__fp16)
...
static inline void fast_sincos_poly_f32_m4_strip(
    ...
    while (remaining > 0) {
        ...
        // Store
        // asm volatile("vse16.v v16, (%0)" :: "r"(poutS) : "memory");
        asm volatile("vse16.v v24, (%0)" :: "r"(poutC) : "memory");
 ...
 *
 * Register plan (LMUL=2, aligned): *   v2  : y | kf | b0f (__fp16)
 *   v4  : x | r (__fp16)
 *   v6  : z = r^2 (__fp16)
 *   v8  : c_acc -> +c0 (__fp16)
 *   v10 : s_acc -> +s0 (__fp16)
...
static inline void fast_sincos_poly_f32_m2_strip(
    const __fp16* inp,  __fp16* outc, int N) {
    ...
    while (remaining > 0) {
        ...
        // Store
        // asm volatile("vse16.v v10, (%0)" :: "r"(poutS) : "memory");
        asm volatile("vse16.v v16, (%0)" :: "r"(poutC) : "memory");
...
int main() {
    ...
    if (cid == 0) {
        g_inp  = (__fp16*)snrt_l1alloc(N * sizeof(__fp16));
        g_outc = (__fp16*)snrt_l1alloc(N * sizeof(__fp16));
        // g_outs = (__fp16*)snrt_l1alloc(N * sizeof(float));
        // if (!g_inp || !g_outc || !g_outs) { printf("alloc failed\n"); return 1; }

        snrt_dma_start_1d(g_inp, data1_dram, N * sizeof(__fp16));
        snrt_dma_wait_all();

        // Initialize outputs (optional)
        memset(g_outc, 0, N * sizeof(__fp16));
    ...
    if (cid == 0) {
        ...
        check_result_fp16(g_inp, g_outc, outS, N);
        // printf("CHECK RESULTS (sin)\n");
        // check_result_fp16(g_inp, g_outs, outS, N);
