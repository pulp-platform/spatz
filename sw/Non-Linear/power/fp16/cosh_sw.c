/*  cosh_sw.c – Schraudolph-based fast cosh(x) for FP16 power evaluation
 *
 *  Algorithm:  cosh(x) = 0.5 * (exp(x) + exp(-x))
 *    where exp(x) ≈ reinterpret_fp16( uint16( B + C·x ) )
 *    C = 2^10 / ln(2) ≈ 1477.32,  B = 15 × 2^10 = 15360
 *
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[COSH_FP16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outC, N, "COSH_FP16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
