/*  tanh_sw.c – Approximate polynomial clamped tanh(x) for FP16 power evaluation
 *
 *  Algorithm:  y = x · (C + x² · (B + A·x²)),  clamped to [-1, +1]
 *    A ≈  0.02661,  B ≈ -0.22852,  C ≈ 0.98047
 *
...
#define FP16_COEFF_A  0x26D0u   /*  0.026611328125  */
#define FP16_COEFF_B  0xB350u   /* -0.228515625     */
#define FP16_COEFF_C  0x3BD8u   /*  0.98046875      */
#define FP16_POS_ONE  0x3C00u   /*  1.0             */
#define FP16_NEG_ONE  0xBC00u   /* -1.0             */

/* ========================= LMUL = 8 ========================= */
static inline void vtanh_m8(const uint16_t *inp, uint16_t *out, int N) {
    float A  = bf16_scalar(FP16_COEFF_A);
    float B  = bf16_scalar(FP16_COEFF_B);
    float C  = bf16_scalar(FP16_COEFF_C);
    float P1 = bf16_scalar(FP16_POS_ONE);
    float N1 = bf16_scalar(FP16_NEG_ONE);
    int rem = N;
    while (rem > 0) {
        size_t vl;
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[TANH_FP16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outT, N, "TANH_FP16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
