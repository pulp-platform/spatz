/*  sin_sw.c – Software sine via Cody-Waite range reduction + cheap polynomial
 *
 *  Algorithm:
 ...
 * Phase 2: reload k, quadrant decode, output sin.
 */
static inline void vsin_m8(const float *inp, float *out, int N) {
    ...
    while (rem > 0) {
        ...
        /* Phase 2: quadrant decode for SIN
         * v8=c0, v16=s0.  sign_s = 1-2*b1,  sin = s0 + b0*(c0-s0) */
        asm volatile(
            "vle32.v    v0,  (%[kbuf])          \n\t"
            "vand.vi    v24, v0,  1             \n\t"   /* b0 */
            "vsrl.vi    v0,  v0,  1             \n\t"
 ...
 */
static inline void vsin_m4(const float *inp, float *out, int N) {
    int rem = N;
    while (rem > 0) {
        size_t vl;
 ...
 */
static inline void vsin_m2(const float *inp, float *out, int N) {
    int rem = N;
    while (rem > 0) {
        size_t vl;
 ...
 */
static inline void vsin_m1(const float *inp, float *out, int N) {
    int rem = N;
    while (rem > 0) {
        size_t vl;
...
static inline void vsin_kernel(const float *inp, float *out, int N) {
#if   LMUL_MODE == 8
    vsin_m8(inp, out, N);
#elif LMUL_MODE == 4
    vsin_m4(inp, out, N);
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[SIN_SW LMUL=%d] cycles=%u  cores=%u  N=%d\n",
