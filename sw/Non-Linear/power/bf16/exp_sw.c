 ...
 * The Spatz VFU extracts rs2[15:0] for e16 scalar operands.
 * BF16 values loaded via flw sit in bits[31:16], leaving [15:0]=0.
 * This helper places the BF16 pattern in [15:0] via fmv.w.x. */
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[EXP_BF16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_bf16(g_out, outE, N, "EXP_BF16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
