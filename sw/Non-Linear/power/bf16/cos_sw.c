/*  cos_sw.c – Software cosine via Cody-Waite range reduction + cheap polynomial (BF16)
 *
 *  Algorithm:
...
#define BF16_C2      0xBEFEu   /* -0.49609375  */
#define BF16_S3      0xBE2Au   /* -0.16601562  */
#define BF16_INVPIO2 0x3F23u   /*  0.63671875  */
#define BF16_PIO2    0x3FC9u   /*  1.5703125   */
#define BF16_ONE     0x3F80u   /*  1.0         */

/* ========================= LMUL = 8 ========================= */
static inline void vcos_m8(const uint16_t *inp, uint16_t *out, int N) {
...
/* ========================= LMUL = 4 ========================= */
static inline void vcos_m4(const uint16_t *inp, uint16_t *out, int N) {
    float INV_val  = bf16_scalar(BF16_INVPIO2);
    float PIO2_val = bf16_scalar(BF16_PIO2);
    float C2_val   = bf16_scalar(BF16_C2);
    float S3_val   = bf16_scalar(BF16_S3);
...
/* ========================= LMUL = 2 ========================= */
static inline void vcos_m2(const uint16_t *inp, uint16_t *out, int N) {
    float INV_val  = bf16_scalar(BF16_INVPIO2);
    float PIO2_val = bf16_scalar(BF16_PIO2);
    float C2_val   = bf16_scalar(BF16_C2);
    float S3_val   = bf16_scalar(BF16_S3);
...
/* ========================= LMUL = 1 ========================= */
static inline void vcos_m1(const uint16_t *inp, uint16_t *out, int N) {
    float INV_val  = bf16_scalar(BF16_INVPIO2);
    float PIO2_val = bf16_scalar(BF16_PIO2);
    float C2_val   = bf16_scalar(BF16_C2);
    float S3_val   = bf16_scalar(BF16_S3);
...
static inline void vcos_kernel(const uint16_t *inp, uint16_t *out, int N) {
#if   LMUL_MODE == 8
    vcos_m8(inp, out, N);
#elif LMUL_MODE == 4
    vcos_m4(inp, out, N);
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[COS_BF16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_bf16(g_out, outCos, N, "COS_BF16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
