/*  log_sw.c – "Cheapest" fast ln(x) for BF16 power evaluation
 *
 *  Algorithm:  ln(x) ≈ bf16( int16(x) - BF16_ONE ) * SCALE
 *    BF16_ONE     = 0x3F80  (bf16 1.0 as integer)
 *    SCALE        = ln(2) / 2^7 ≈ 0.005402 (BF16: 0x3BB1)
 *
...
static const unsigned int MAGIC_BIAS = 0x3F80u;  /* integer path — OK */
#define BF16_SCALE 0x3BB1u  /* 0.005402 */

/* ========================= LMUL = 8 ========================= */
static inline void vlog_m8(const uint16_t *inp, uint16_t *out, int N) {
    float sc = bf16_scalar(BF16_SCALE);
    int rem = N;
    while (rem > 0) {
        size_t vl;
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[LOG_BF16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_bf16(g_out, outL, N, "LOG_BF16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
