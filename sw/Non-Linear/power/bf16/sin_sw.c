/*  sin_sw.c – Software sine via Cody-Waite range reduction + cheap polynomial (BF16)
 *
 *  Algorithm:
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[SIN_BF16 LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_bf16(g_out, outS, N, "SIN_BF16");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
