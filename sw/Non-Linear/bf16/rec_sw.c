/*  rec_sw.c – Newton-Raphson reciprocal for BF16
 *
 *  Algorithm: x0 = as_float(M - as_int(a)), then 2 NR iterations
 *  BF16 M = 0x7EFF, two = 2.0
...
/* BF16 constants */
#define BF16_M    0x7EFFu   /* magic constant for reciprocal */
#define BF16_TWO  0x4000u   /* 2.0 in bf16 */

/* ========================= LMUL = 8 ========================= */
static inline void vrec_m8(const uint16_t *inp, uint16_t *out, int N) {
    float two_val = bf16_scalar(BF16_TWO);
    int rem = N;
    while (rem > 0) {
        size_t vl;
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        check_bf16(g_out, outRec, N, "REC_SW");
    }
