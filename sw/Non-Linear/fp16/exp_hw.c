/*  exp_hw.c – Hardware exponential via vfexpf.v burst pattern (FP16)
 *
 *  Algorithm:  4-way unrolled load → vfexpf.v → store
 *  The HW instruction computes exp(x) in the non-linear pipeline.
 *
...
/* ========================= LMUL = 8 ========================= */
static inline void vexp_hw_m8(const uint16_t *inp, uint16_t *out, int N) {
    size_t vl;
...
/* ========================= LMUL = 4 ========================= */
static inline void vexp_hw_m4(const uint16_t *inp, uint16_t *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfexpf.v v0,  v0");
        asm volatile("vfexpf.v v4,  v4");
        asm volatile("vfexpf.v v8,  v8");
        asm volatile("vfexpf.v v12, v12");
        asm volatile("vse16.v  v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 2 ========================= */
static inline void vexp_hw_m2(const uint16_t *inp, uint16_t *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfexpf.v v0,  v0");
        asm volatile("vfexpf.v v2,  v2");
        asm volatile("vfexpf.v v4,  v4");
        asm volatile("vfexpf.v v6,  v6");
        asm volatile("vse16.v  v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 1 ========================= */
static inline void vexp_hw_m1(const uint16_t *inp, uint16_t *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vfexpf.v v0, v0");
        asm volatile("vfexpf.v v1, v1");
        asm volatile("vfexpf.v v2, v2");
        asm volatile("vfexpf.v v3, v3");
        asm volatile("vse16.v  v0, (%0)" :: "r"(out)           : "memory");
...
static inline void vexp_hw_kernel(const uint16_t *inp, uint16_t *out, int N) {
#if   LMUL_MODE == 8
    vexp_hw_m8(inp, out, N);
#elif LMUL_MODE == 4
    vexp_hw_m4(inp, out, N);
#elif LMUL_MODE == 2
    vexp_hw_m2(inp, out, N);
#elif LMUL_MODE == 1
    vexp_hw_m1(inp, out, N);
#else
...
int main(void) {
    ...
    if (len > 0) vexp_hw_kernel(g_inp + start, g_out + start, len);
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[EXP_HW LMUL=%d] cycles=%u  cores=%u  N=%d\n",
               LMUL_MODE, cyc, cores, N);
        check_fp16(g_out, outE, N, "EXP_HW");
    }
    snrt_cluster_hw_barrier();
    return 0;
}
