#!/usr/bin/env bash
# power_sweep.sh – Automated VCD → PrimeTime power analysis for all FP16 kernels
#
# Usage:  bash power_sweep.sh [tests.list]
#   If no argument, generates tests.list from all 64 binaries, then runs the sweep.
#
# Prerequisites:
#   1) FP16 power binaries built (cmake --build ... in spatz_cluster/sw/build)
...
# Build directory for the power-fp16 tests
BUILD_DIR="${TOP_DIR}/spatz/hw/system/spatz_cluster/sw/build/power_fp16"
TEST_PREFIX="test-power-fp16-"
...
if [[ -z "${TEST_LIST_FILE}" ]]; then
    TEST_LIST_FILE="${GF12_DIR}/tests_power_fp16.list"
    echo "# Auto-generated power-fp16 test list" > "${TEST_LIST_FILE}"
