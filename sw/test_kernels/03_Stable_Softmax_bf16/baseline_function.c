...
#define H_SCH_C 0x4339u
#define H_SCH_B 0x467Eu

/* ---- Newton-Raphson reciprocal ---- */
#define H_REC_MAGIC 0x7EFFu
...
static uint16_t *g_inv_sum;       /* 1 float – broadcast to all cores */
...
static void pass1_local_max(const uint16_t *src, int len, int cid) {
    float neg_inf = h_scalar(0xFF80u);
    ...
    while (rem > 0) {
        ...
        asm volatile(
            ...
            : [src]"r"(src), [dst]"r"(dst), [mx]"f"(max_val),
              [B]"f"(h_scalar(H_SCH_B)), [C]"f"(h_scalar(H_SCH_C))
            : "memory",
              "v0","v1","v2","v3",
              "v4","v5","v6","v7",
              "v12"
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        float y = seed.f;
        y = y * (2.0f - total_sum_f32 * y);   /* NR step 1 */
        y = y * (2.0f - total_sum_f32 * y);   /* NR step 2 */
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[SOFTMAX 2-CORE BF16] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
