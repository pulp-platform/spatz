/*  RMSNorm baseline – Root Mean Square Normalization using SW rsqrt (FP16)
 *
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        float threehalves = h_scalar(0x3E00u);
        float half_h = h_scalar(0x3800u);
        asm volatile(
            "vsetivli zero, 1, e16, m1, ta, ma \n\t"
            "vfmv.v.f v0, %[ms] \n\t"
            "vfmul.vf v0, v0, %[hx] \n\t"
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RMSNORM 2-CORE FP16] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
