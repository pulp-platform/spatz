 ...
 *  ~14 bits (~4 decimal digits), close to full FP16 mantissa precision
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        /* 1 NR step in 16-bit via vector ops */
        { float nr_threehalves = h_scalar(0x3E00u);
        float nr_half = h_scalar(0x3800u);
        asm volatile(
            "vsetivli zero, 1, e16, m1, ta, ma \n\t"
            "vfmv.v.f v0, %[ms] \n\t"
            "vfmul.vf v0, v0, %[hx] \n\t"
            ...
            "vfmv.f.s %[r], v1 \n\t"
            : [r]"=f"(y)
            : [ms]"f"(mean_sq), [y]"f"(y), [hx]"f"(nr_half), [th]"f"(nr_threehalves)
            : "v0", "v1", "v2"
        ); }
        asm volatile(
            "vsetivli zero, 1, e16, m1, ta, ma \n\t"
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RMSNORM CUSTOM 2-CORE FP16] cycles=%u  cores=%u  N=%d\n",
               cyc, SM_CORES, N);
