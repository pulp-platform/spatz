/*  RoPE baseline – Rotary Positional Embedding using SW sin/cos (FP16)
 *
 *  Kernel:
...
#define H_CW_C2 0xB7F2u
#define H_CW_S3 0xB150u
#define H_CW_INVPIO2 0x3918u
#define H_CW_PIO2 0x3E48u

/* ---- RoPE frequency base ---- */
#define H_ROPE_BASE 0x70E2u
    ...
    while (rem > 0) {
        ...
        asm volatile(
            ...
            : [th]"r"(theta), [xe]"r"(x_even), [xo]"r"(x_odd),
              ...
              [S3]"f"(h_scalar(H_CW_S3)), [C2]"f"(h_scalar(H_CW_C2)), [ONE]"f"(h_scalar(0x3C00u))
            : "memory"
        );
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[RoPE BASELINE FP16] cycles=%u  cores=%u  N=%d\n", cyc, cores, N);
