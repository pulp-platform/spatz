/*  log_hw.c – Hardware log via vflog.v burst pattern
 *
 *  Algorithm:  4-way unrolled load → vflog.v → store
 *  The HW instruction computes log(x) in the non-linear pipeline.
 *
 ...
 */
static inline void vlog_hw_m8(const float *inp, float *out, int N) {
    size_t vl;
...
/* ========================= LMUL = 4 ========================= */
static inline void vlog_hw_m4(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vflog.v v0,  v0");
        asm volatile("vflog.v v4,  v4");
        asm volatile("vflog.v v8,  v8");
        asm volatile("vflog.v v12, v12");
        asm volatile("vse32.v v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 2 ========================= */
static inline void vlog_hw_m2(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vflog.v v0,  v0");
        asm volatile("vflog.v v2,  v2");
        asm volatile("vflog.v v4,  v4");
        asm volatile("vflog.v v6,  v6");
        asm volatile("vse32.v v0,  (%0)" :: "r"(out)           : "memory");
...
/* ========================= LMUL = 1 ========================= */
static inline void vlog_hw_m1(const float *inp, float *out, int N) {
    size_t vl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        asm volatile("" ::: "memory");
        asm volatile("vflog.v v0, v0");
        asm volatile("vflog.v v1, v1");
        asm volatile("vflog.v v2, v2");
        asm volatile("vflog.v v3, v3");
        asm volatile("vse32.v v0, (%0)" :: "r"(out)           : "memory");
...
static inline void vlog_hw_kernel(const float *inp, float *out, int N) {
#if   LMUL_MODE == 8
    vlog_hw_m8(inp, out, N);
#elif LMUL_MODE == 4
    vlog_hw_m4(inp, out, N);
#elif LMUL_MODE == 2
    vlog_hw_m2(inp, out, N);
#elif LMUL_MODE == 1
    vlog_hw_m1(inp, out, N);
#else
...
int main(void) {
    ...
    if (len > 0) vlog_hw_kernel(g_inp + start, g_out + start, len);
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[LOG_HW LMUL=%d] cycles=%u  cores=%u  N=%d\n",
