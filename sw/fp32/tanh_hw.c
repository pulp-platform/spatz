/*  tanh_hw.c – Hardware tanh via vftanhf.v burst pattern
 *
 *  Algorithm:  4-way unrolled load → vftanhf.v → store
 *  The HW instruction computes tanh(x) in the non-linear pipeline.
 *
 ...
 */
static inline void vtanh_hw_m8(const float *inp, float *out, int N) {
    size_t vl;
...
/* ========================= LMUL = 4 ========================= */
static inline void vtanh_hw_m4(const float *inp, float *out, int N) {
    size_t vl;
...
/* ========================= LMUL = 2 ========================= */
static inline void vtanh_hw_m2(const float *inp, float *out, int N) {
    size_t vl;
...
/* ========================= LMUL = 1 ========================= */
static inline void vtanh_hw_m1(const float *inp, float *out, int N) {
    size_t vl;
...
static inline void vtanh_hw_kernel(const float *inp, float *out, int N) {
#if   LMUL_MODE == 8
...
int main(void) {
    ...
    if (cid == 0) {
        ...
        stop_kernel();
        printf("[TANH_HW LMUL=%d] cycles=%u  cores=%u  N=%d\n",
