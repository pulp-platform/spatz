...
#include "widening4x-bp-matmul-quadrilatero.h"
...
void  __attribute__ ((noinline))  matrixMul_16x16(int8_t* addrA,int8_t* addrB,int8_t* addrC, int K, int N, int M, int shift)
{
    asm volatile("addi	sp, sp, -0x30           "                            );   // 
    asm volatile("sw	s0 , 0x2c(sp)           "                            );   // 
    asm volatile("sw	s1 , 0x28(sp)           "                            );   // 
