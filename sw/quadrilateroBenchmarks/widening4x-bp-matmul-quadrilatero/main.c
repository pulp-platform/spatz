...
#include DATAHEADER
#include "kernel/widening4x-bp-matmul-quadrilatero.c"
...
int check_results(int32_t *matrix, const int32_t *expected, int N, int M)
{
  // check
  int i, j;
  int err = 0;
...
int main() {
  ...
  if (cid == 0) {
    a = (int8_t  *)snrt_l1alloc(gemm_l.M * gemm_l.K * sizeof(int8_t ));
    b = (int8_t  *)snrt_l1alloc(gemm_l.K * gemm_l.N * sizeof(int8_t ));
    c = (int32_t *)snrt_l1alloc(gemm_l.M * gemm_l.N * sizeof(int32_t));
  }
  ...
  // Initialize matrices
  if (cid == 0) {
    snrt_dma_start_1d(a, gemm_A_dram, gemm_l.M * gemm_l.K * sizeof(int8_t));
    snrt_dma_start_1d(b, gemm_B_dram, gemm_l.K * gemm_l.N * sizeof(int8_t));
  ...
  for (unsigned int i = 0; i < measure_iterations; ++i) {
    ...
    if (kernel_size == 4) {
      matrixMul_8x8(a,b,c,gemm_l.K/4,gemm_l.N,gemm_l.M,0);
    } else if (kernel_size == 8) {
      matrixMul_16x16(a,b,c,gemm_l.K/4,gemm_l.N,gemm_l.M,0);
    } else {
      return -2;
    }
  ...
  if (cid == 0) {
    int error =
      check_results(c, (const int32_t *)gemm_EXP_dram, gemm_l.M, gemm_l.N);
