...
#include "widening4x-bp-fmatmul-quadrilatero.h"
...
void __attribute__ ((noinline)) matrixMul_8x8(char* addrA,char* addrB,float* addrC, int K, int N, int M, int shift)
{
    ...
    asm volatile("fmmacc.b m4,m1,m0             "                            );   // m4 += m1 * m0
    ...
    asm volatile("fmmacc.b m6,m1,m2              "                            );   // m6 +=  m1 * m2
    ...
    asm volatile("fmmacc.b m5,m3,m0              "                            );   // m5 +=  m3 * m0
    ...
    asm volatile("fmmacc.b m7,m3,m2              "                            );   // m7 += m3 * m2
