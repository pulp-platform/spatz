...
#include "widening2x-hp-matmul-quadrilatero.h"
...
void __attribute__ ((noinline)) matrixMul_8x8(int16_t* addrA,int16_t* addrB,int32_t* addrC, int K, int N, int M, int shift)
{
    ...
    asm volatile("mmada.h m4,m1,m0              "                            );   // m4 += m1 * m0
    ...
    asm volatile("mmada.h m6,m1,m2              "                            );   // m6 +=  m1 * m2
    ...
    asm volatile("mmada.h m5,m3,m0              "                            );   // m5 +=  m3 * m0
    ...
    asm volatile("mmada.h m7,m3,m2              "                            );   // m7 += m3 * m2
...
void  __attribute__ ((noinline))  matrixMul_16x16(int16_t* addrA,int16_t* addrB,int16_t* addrC, int K, int N, int M, int shift)
{
    asm volatile("addi	sp, sp, -0x30           "                            );   // 
    asm volatile("sw	s0 , 0x2c(sp)           "                            );   // 
    asm volatile("sw	s1 , 0x28(sp)           "                            );   // 
