...
#include "widening2x-hp-fmatmul-quadrilatero.h"
...
void __attribute__ ((noinline)) matrixMul_8x8(__fp16* addrA,__fp16* addrB,float* addrC, int K, int N, int M, int shift)
{
    ...
    asm volatile("fmmacc.h m4,m1,m0             "                            );   // m4 += m1 * m0
    ...
    asm volatile("fmmacc.h m6,m1,m2              "                            );   // m6 +=  m1 * m2
    ...
    asm volatile("fmmacc.h m5,m3,m0              "                            );   // m5 +=  m3 * m0
    ...
    asm volatile("fmmacc.h m7,m3,m2              "                            );   // m7 += m3 * m2
...
void  __attribute__ ((noinline))  matrixMul_16x16(__fp16* addrA,__fp16* addrB,__fp16* addrC, int K, int N, int M, int shift)
{
    asm volatile("addi	sp, sp, -0x30           "                            );   // 
    asm volatile("sw	s0 , 0x2c(sp)           "                            );   // 
    asm volatile("sw	s1 , 0x28(sp)           "                            );   // 
