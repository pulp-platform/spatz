...
void __attribute__ ((noinline)) matrixMul_8x8(int* addrA,int* addrB,int* addrC, int K, int N, int M, int shift)
{
    ...
    asm volatile("mmasa.w m4,m1,m0              "                            );   // m4 += m1 * m0
    ...
    asm volatile("mmasa.w m6,m1,m2              "                            );   // m6 +=  m1 * m2
    ...
    asm volatile("mmasa.w m5,m3,m0              "                            );   // m5 +=  m3 * m0
    ...
    asm volatile("mmasa.w m7,m3,m2              "                            );   // m7 += m3 * m2
...
void  __attribute__ ((noinline))  matrixMul_16x16(int* addrA,int* addrB,int* addrC, int K, int N, int M, int shift)
{
    asm volatile("addi	sp, sp, -0x30           "                            );   // 
    asm volatile("sw	s0 , 0x2c(sp)           "                            );   // 
    asm volatile("sw	s1 , 0x28(sp)           "                            );   // 
