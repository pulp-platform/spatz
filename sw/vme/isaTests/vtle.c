...
// TC1: vtle8 row load — uint8 zero-extended to int32
//
//   src[4] = {10, 20, 30, 40}  (uint8)
//   vtle8 tss=row(0,0), base=src
//   tile[0][0][0..3] should be [10, 20, 30, 40] as int32
// -------------------------------------------------------
...
//
//   src[4] = {5, 6, 7, 8}  (uint8)
//   TSS = col(tile=0, col=0) = 0x01000000
//   tile[0][0..3][0] = [5, 6, 7, 8]
//   Read back via vtmv.v.t with col pattern
// -------------------------------------------------------
void TEST_CASE4(void) {
    ...
    uintptr_t tss  = TSS_COL(0, 0);   // tile=0, pattern=col, col_idx=0
    uintptr_t base = (uintptr_t)src;
    asm volatile("vtle8 %[tss], (%[base])"
                 :: [tss]"r"(tss), [base]"r"(base) : "memory");
...
//
//   Load rows 0-3 from separate memory locations.
...
// -------------------------------------------------------
// TC6: vtle8 tile isolation
//   Load a row into mt0. mt4 must remain zero.
//   TSS for mt4 row 0 = 0x20000000
// -------------------------------------------------------
void TEST_CASE6(void) {
    ...
    uintptr_t tss  = TSS_ROW(0, 0);   // mt0 row 0
    uintptr_t base = (uintptr_t)src;
    asm volatile("vtle8 %[tss], (%[base])"
                 :: [tss]"r"(tss), [base]"r"(base) : "memory");
...
//   Load int32 row → store back → compare with original
// -------------------------------------------------------
