...
// main.c - fp32 x fp32 = fp32 VME GEMM functional check
...
_Static_assert(BUFFER_BYTES <= L1_BUFFER_STRIDE,
               "one M64xN64xK128 TEW32 buffer must fit in 128 KiB");
_Static_assert(L1_BYTES <= TCDM_BYTES,
               "two matmul buffers exceed the 256 KiB TCDM");
...
static snrt_dma_txid_t fill_panel(float *dst, const float *src,
    ...
    const size_t copy_bytes = valid_k * TE * sizeof(float);
    const size_t src_stride = src_tile_k * TE * sizeof(float);
...
int main(void) {
    ...
    partials = (float *)snrt_l3alloc((size_t)iterations * C_BYTES);
    ...
    printf("\n=== TEW32 FP32 matmul %ux%ux%u "
           "(M%uxN%uxK%u chunks, %u buffers, 1 core) ===\n",
           matmul_l.M, matmul_l.N, matmul_l.K,
           M_CHUNK, N_CHUNK, K_CHUNK, L1_BUFFER_COUNT);
    printf("Cycles:    %u\n", cycles);
