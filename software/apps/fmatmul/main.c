...
float a[M * N] __attribute__((aligned(4), section(".l1")));
float b[N * P] __attribute__((aligned(4), section(".l1")));
float c[M * P] __attribute__((aligned(4), section(".l1")));

// Initialize the matrices
void init_matrix(float *matrix, const unsigned int rows_start,
                 const unsigned int rows_end, const unsigned int num_columns,
                 int a, int b, int c) {
  for (unsigned int i = rows_start; i < rows_end; ++i) {
    for (unsigned int j = 0; j < num_columns; ++j) {
      matrix[i * num_columns + j] = (float)(a * (int)i + b * (int)j + c);
    }
  }
}

// Verify the matrices
int verify_matrix(float const *matrix, const unsigned int row_start,
