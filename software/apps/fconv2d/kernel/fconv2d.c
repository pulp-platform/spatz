...
void conv3d_CHx7x7_block(double *o, const double *i, unsigned int nrows,
                         const double *f, const unsigned int n_) {

  // Helper variables
  const int ldo = N << 3;
  const int ldi_pad = (N + F - 1) << 3;
  ...
  // store!)
  int last_f_column = (C - 1) * fch_len + F - 1;
  ...
  for (int ch = 0; ch < C; ++ch) {
    ...
    for (int k = 0; k < F / 2; ++k) {
      ...
      const int base_idx_0 = (2 * k) + (ch * fch_len);
      // Point to the first element of the current column (k+1) of the current
      // channel (ch) of the filter (f)
      const int base_idx_1 = (2 * k + 1) + (ch * fch_len);
    ...
    const int base_idx_0 = (F - 1) + (ch * fch_len);
  ...
  // 2, just for easeness)
  for (unsigned int j = 0; j < ((nrows + F - 1) - 2 * F) / 2; ++j) {
    ...
    for (int ch = 0; ch < C; ++ch) {
      ...
      for (int k = 0; k < F / 2; ++k) {
        ...
        const int base_idx_0 = (2 * k + 2) + (ch * fch_len);
        // Point to the first element of the current column (k+1) of the current
        // channel (ch) of the filter (f)
        const int base_idx_1 = (2 * k + 1) + (ch * fch_len);
      ...
      if (ch != C - 1) {
        const int base_idx_0 = (ch + 1) * fch_len;
